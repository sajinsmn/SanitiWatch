...
const AdminRegistrationPage = () => {
  ...
  useEffect(() => {
    const fetchExistingData = async () => {
      try {
        ...
        if (user && user.id) {
          const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/profile/admin/${user.id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.fullName || !formData.roleTitle || !formData.address || !formData.phone) {
    ...
    try {
      ...
      await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/admin/register`, {
      ...
      <div className="registration-container" style={{ paddingTop: '100px' }}>
        <div className="registration-card">
          <div className="registration-header">
            ...
            <p>Provide your details to access the admin dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="worker-registration-form">
            ...
            <div className="form-group">
              ...
              <input type="text" id="roleTitle" name="roleTitle" value={formData.roleTitle} onChange={handleChange} required className="form-input" />
            </div>
            <div className="form-group">
